...
export default function BookSlot() {
  ...
  const loadParkingDetails = async () => {
    try {
      setLoading(true);
      console.log("🔍 Loading parking details for ID:", parkingSystemId);
      console.log("🔍 Property ID details:", {
        id: parkingSystemId,
        length: parkingSystemId?.length,
        type: typeof parkingSystemId,
        isValid24Char: parkingSystemId?.length === 24
      });
      
      // Get parking property details
      const property = await apiRequest(`/parking-property/${parkingSystemId}`, "GET");
      console.log("🏢 Parking property loaded:", property);
      console.log("🎨 Layout data check:", {
        hasLayoutData: !!property.layoutData,
        layoutData: property.layoutData,
        templateName: property.layoutData?.templateName,
      ...
      console.log("🅿️ Available slots loaded:", slots);
  ...
  const debugLayoutConsistency = async () => {
    try {
      console.log("🔍 Starting layout debug for property ID:", parkingSystemId);
      
      if (!parkingSystemId) {
        setDebugInfo({ error: "No property ID available" });
        return;
      }
      
      // Validate property ID format
      if (parkingSystemId.length !== 24) {
        setDebugInfo({ 
          error: `Invalid property ID format. Expected 24 characters, got ${parkingSystemId.length}. ID: ${parkingSystemId}` 
        });
        return;
      }
      
      console.log("📡 Making debug API call to:", `/parking-property/${parkingSystemId}/debug-layout`);
      
      const result = await apiRequest(`/parking-property/${parkingSystemId}/debug-layout`, "GET");
      console.log("✅ Layout Debug Result:", result);
      setDebugInfo(result);
      ...
      setDebugInfo({ 
        ...
        details: {
          propertyId: parkingSystemId,
          endpoint: `/parking-property/${parkingSystemId}/debug-layout`,
        ...
        {parkingProperty && (
          <div className="property-info">
            ...
            <div className="debug-section">
              <button 
                type="button" 
                onClick={debugLayoutConsistency}
                className="btn-debug"
                style={{
                  background: '#6c757d',
                  color: 'white',
                  ...
                  {debugInfo.success ? (
                    <div>
                      ...
                      <ul style={{ marginLeft: '15px', marginTop: '5px' }}>
                        <li>Consistency Score: {debugInfo.analysis.consistencyScore}%</li>
                        <li>Has Layout Data: {debugInfo.analysis.hasLayoutData ? '✅' : '❌'}</li>
                        <li>Layout Slots: {debugInfo.analysis.layoutAnalysis.slotsCount || 0}</li>
                        <li>Template: {debugInfo.analysis.layoutAnalysis.templateName}</li>
                        {debugInfo.analysis.layoutAnalysis.consistencyIssues?.length > 0 && (
                          <li style={{ color: '#dc3545' }}>
                            Issues: {debugInfo.analysis.layoutAnalysis.consistencyIssues.join(', ')}
                          </li>
                        )}
                      </ul>
